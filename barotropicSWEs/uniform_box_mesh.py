...
def main(
    ...
    if not file_exist(f"{folder}/{file_name}.npz"):
        ...
        dem = DEM(h_func, bbox=extent.bbox)  # creates a DEM object for own function, h_func
        smoothing = False
        print("creating shoreline")
        if len(edgefuncs) == 1 and edgefuncs[0] == "UNIFORM":
            shore = Shoreline(fname, extent_.bbox, h_max, smooth_shoreline=smoothing)

        else:
            shore = Shoreline(fname, extent_.bbox, h_min, smooth_shoreline=smoothing)
        ...
        if "DISTANCING" in edgefuncs:
            edge_lengths.append(distance_sizing_function(shore, max_edge_length=h_max))
        ...
        if "SLOPING" in edgefuncs:
            edge_lengths.append(bathymetric_gradient_sizing_function(dem, slope_parameter=slp, filter_quotient=fl, min_edge_length=h_min))
        ...
        edge_length = enforce_mesh_gradation(edge_length, gradation=0.2)
