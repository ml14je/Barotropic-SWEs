...
_{background_flow_}_ω={ω:.1f}_{variable}={1e-3*value:.0f}"
        ...
        if plot_sects:
            ...
            for i in range(len(labels)):
                ...
                phase = np.angle(
                    sects[0][labels[i]][np.argmax(abs(sects[0][labels[i]]))]
                )
                ax.plot(
                    sects["grid"] * 1e-3,
                    prefactor
                    * np.array(
                        [
                            np.round(sects[w_][labels[i]] * np.exp(-1j * phase), 16)
                            for w_ in w_vals_
                        ]
                    ).real.T,
                    "-",
                ...
                ax.legend(
                    [f"{w*param.L_R*1e-3 :.0f}" for w in w_vals_],
                    title="Canyon Width (km):",
                    fontsize=16,
                    title_fontsize=18,
                    loc=1,
                )
...
if __name__ == "__main__":
    ...
    sols_dict = {}
    for domain_width in [0.05, 0.1, 0.15]:
        ...
        for var_ in ["x", "y"]:
            if var_ == "x":
                values_ = [-0.025, 0, 0.025]
            else:
                values_ = [0.01, 0.02, 0.03, 0.05]

            for value_ in values_:
                dictionary = canyon_analysis(
                    bbox,
                    param,
                    order,
                    h_min,
    ...
    L_vals_ = np.array([0.05, 0.1, 0.15])
            ...
            for var_ in ["x", "y"]:
                ...
                for value_ in values_:
                    ...
                    for i, w_ in enumerate(w_vals_):
                        ...
                        phases = [
                            np.angle(vals[np.argmax(np.abs(vals))]) for L in L_vals_
                        ]
