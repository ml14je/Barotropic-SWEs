...
def yield_data(swes, sols, bbox_plot=(-0.025, 0.025, 0, 0.05)):
    """
...
def canyon_analysis(
 ...
 _{background_flow_}_ω={ω:.1f}"
        ...
        if plot_norms:
            ...
            for label in labels:
                fig, ax = plot_setup("Canyon Width ($\\rm{km}$)", f"{norm}-Norm")
                ax.plot(
                    w_vals[1:] * param.L_R / 1e3,
                    [norms[w_][label] for w_ in w_vals[1:]],
                    "-",
                    c="k",
                )
                ax.scatter(
        ...
        for k, label in enumerate(
            ...
            for i, norm_ in enumerate(["L1", "L2", "LINF"]):
                ...
                for domain_width in [0.05, 0.1, 0.15]:
                    ...
                    vals = [
                        sols_dict[domain_width][norm_][j][w_][label] * prefactor
                        for w_ in w_vals
                    ]
